...
function Plans() {
  ...
  const browseTotalPages = Math.ceil(activePlans.length / PLANS_PER_PAGE);
  const pagedBrowsePlans = activePlans.slice(
    (browsePage - 1) * PLANS_PER_PAGE,
    browsePage * PLANS_PER_PAGE
  );
  const compareTotalPages = Math.ceil(activePlans.length / PLANS_PER_PAGE);
  const pagedComparePlans = activePlans.slice(
    (comparePage - 1) * PLANS_PER_PAGE,
    comparePage * PLANS_PER_PAGE
  );
      ...
      >
        ...
        <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>
          ...
          <strong style={{ color: "var(--text-dark)" }}>
            {(currentPage - 1) * PLANS_PER_PAGE + 1}–
            {Math.min(currentPage * PLANS_PER_PAGE, activePlans.length)}
          </strong>{" "}
          of{" "}
          <strong style={{ color: "var(--text-dark)" }}>
            {activePlans.length}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          <button
            ...
            style={{
              ...
              background:
                currentPage === 1 ? "var(--bg-subtle)" : "var(--bg-card)",
              border: "1.5px solid var(--border)",
              borderRadius: "8px",
              cursor: currentPage === 1 ? "not-allowed" : "pointer",
              color:
                currentPage === 1 ? "var(--text-light)" : "var(--text-mid)",
              fontSize: "15px",
              fontWeight: "600",
              ...
              {activeTab === "browse" && (
                <>
                  <div className="plans-grid">
                    {pagedBrowsePlans.map((plan, index) => {
              ...
              {activeTab === "compare" && (
                <div className="data-table-card">
                  ...
                  <table>
                    ...
                    <tbody>
                      {pagedComparePlans.map((plan, index) => {
                        const planIdFromUser = String(
                          currentSub?.planId || "",
                        );
                        const planIdFromList = String(plan.id || "");
                        const isCurrent =
